                    ...
                    <form action="{{ route('admin.items.update', $item) }}" method="POST" enctype="multipart/form-data">
                            ...
                            @if($item->image_path)
                                <div class="mb-2">
                                    <img src="{{ Str::startsWith($item->image_path, 'http') ? $item->image_path : Storage::url($item->image_path) }}"
                                        alt="{{ $item->name }}" class="h-20 w-20 object-cover rounded">
                                </div>
                            @endif
                        ...
                        <div class="mb-6 border-t pt-4">
                            ...
                            <div id="variations-wrapper" class="space-y-4">
                                @foreach($item->variations as $vIndex => $variation)
                                    <div class="border rounded p-4 bg-gray-50 relative" id="variation-{{ $vIndex }}">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
                                            <div>
                                                ...
                                                <input type="text" name="variations[{{ $vIndex }}][name]"
                                                    value="{{ $variation->name }}" placeholder="e.g. Size"
                                                    class="shadow border rounded w-full py-1 px-2 text-sm" required>
                                            </div>
                                            <div>
                                                <label class="block text-xs font-bold mb-1">Type</label>
                                                <select name="variations[{{ $vIndex }}][type]"
                                                    class="shadow border rounded w-full py-1 px-2 text-sm">
                                                    <option value="radio" {{ $variation->type == 'radio' ? 'selected' : '' }}>
                                                        Single Selection (Radio)</option>
                                                    <option value="checkbox" {{ $variation->type == 'checkbox' ? 'selected' : '' }}>Multiple Selection (Checkbox)</option>
                                                    <option value="select" {{ $variation->type == 'select' ? 'selected' : '' }}>Dropdown</option>
                                                </select>
                                            </div>
                                            <div class="flex items-center mt-6">
                                                <input type="hidden" name="variations[{{ $vIndex }}][required]" value="0">
                                                <input type="checkbox" name="variations[{{ $vIndex }}][required]" value="1"
                                                    class="mr-2" {{ $variation->required ? 'checked' : '' }}>
                                        ...
                                        <div class="ml-4 border-l-2 border-gray-300 pl-4 mt-2">
                                            ...
                                            <div id="options-wrapper-{{ $vIndex }}" class="space-y-2">
                                                @foreach($variation->options as $oIndex => $option)
                                                    <div class="flex items-center space-x-2">
                                                        <input type="text"
                                                            name="variations[{{ $vIndex }}][options][{{ $oIndex }}][name]"
                                                            value="{{ $option->name }}" placeholder="Option Name"
                                                            class="shadow border rounded w-1/2 py-1 px-2 text-sm" required>
                                                        <input type="number" step="0.01"
                                                            name="variations[{{ $vIndex }}][options][{{ $oIndex }}][price]"
                                                            value="{{ $option->price }}" placeholder="Price"
                                                            class="shadow border rounded w-1/3 py-1 px-2 text-sm" required>
                                                        <button type="button" onclick="this.parentElement.remove()"
                                                            class="text-red-500 font-bold ml-2">&times;</button>
                                                    </div>
                                                @endforeach
                                            </div>
                                            <button type="button" onclick="addOption({{ $vIndex }})"
                                                class="mt-2 text-blue-500 text-xs font-bold hover:underline">+ Add
                                                Option</button>
                                        </div>
                                    </div>
                                @endforeach
                        ...
                        <script>
                            let variationCount = {{ $item->variations->count() }};
                            ...
                            function addOption(variationIndex) {
                                ...
                                const count = new Date().getTime(); // Unique ID for key, though PHP array parsing handles sequential index gaps usually, better to be safe with just unique keys or just append
                                const html = `
                                    <div class="flex items-center space-x-2">
                                        <input type="text" name="variations[${variationIndex}][options][${count}][name]" placeholder="Option Name" class="shadow border rounded w-1/2 py-1 px-2 text-sm" required>
                                        <input type="number" step="0.01" name="variations[${variationIndex}][options][${count}][price]" placeholder="Price" class="shadow border rounded w-1/3 py-1 px-2 text-sm" required>
